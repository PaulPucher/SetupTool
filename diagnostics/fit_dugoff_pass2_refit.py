# WP-N2 pass 2: Dugoff curve refit from the EKF's OWN slip angles
# (pass_1, the calibrated configuration), closing one iteration of the
# kinematic-seeded-fit -> observer -> refit loop named in WP-N1b.
# Read-only w.r.t. production; writes only this script's own manifest.
# Tier B calibration/fit exercise, same status as fit_dugoff_first_pass.py.
#
# NEW FILE rather than editing fit_dugoff_first_pass.py in place: that
# script is the WP-N1/WP-N1b KINEMATIC-sourced fit, and its manifest
# (timestamp 2026-08-19T14:00:40Z) is already pointed to by config's
# tyre_model_ekf.pass_0.frozen_from. Mutating it would break the
# reproducibility the config's own _comment demands ("a numbered pass
# must be fully reproducible from its own recorded parameters alone").
#
# Two-step fit per axle, same shape as WP-N1b, different inputs:
#   1. c_alpha: median of Module 4b's C_alpha_f/C_alpha_r -- computed by
#      re-running estimate_cornering_stiffness on the EKF's OWN alpha/Fy
#      (not the kinematic path's CS_ratio==1.0 flag, which is itself
#      kinematic-alpha-derived and would launder the kinematic under-read
#      into this refit's own sample selection) -- over this script's base
#      mask intersected with THAT EKF-alpha CS_ratio's own ==1.0 flag.
...
#      1, using every masked sample.
#
# Slip source: the EKF's own alpha (diagnostics/sideslip_ekf_dugoff.py,
# pass_id="pass_1" -- the calibrated setting, not pass_0 which is
# known-miscalibrated in R and carries the C2 excursion pathology pass_1
# already fixed).
...
MANIFEST_PATH = "diagnostics/fit_dugoff_pass2_refit_manifest.json"
EKF_SOURCE_PASS_ID = "pass_1"
...
print("=" * 100)
print(f"WP-N2 pass 2 -- Dugoff refit, slip angles from EKF {EKF_SOURCE_PASS_ID}")
print("=" * 100)
...
manifest = {
    ...
    "c_alpha_source": "module_4b median over CS_ratio==1.0 linear-regime samples, "
                       ...
                       "(not the kinematic path's CS_ratio flag) -- WP-N2 pass 2",
