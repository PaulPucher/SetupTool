# WP-N2 pass 3 evaluation: relative parameter change vs pass 2, the
# mu_fz/c_alpha ratio trend across pass_0/pass_2/pass_3 (the ridge check),
# NIS exceedance, sign check and the h2-vs-ay apex-population check.
# Mirrors inspect_ekf_pass2_evaluation.py's structure with the
# comparison baseline moved to pass_2 and the ratio-trend section added.
...
result_3 = estimate_sideslip_ekf_dugoff(state, params, pass_id=PASS_ID)
beta_3 = result_3["beta"]
slip_3 = estimate_slip_angles(state, beta_3, params)

cfg3 = params["tyre_model_ekf"][PASS_ID]
cfg2 = params["tyre_model_ekf"][PRIOR_PASS_ID]
cfg0 = params["tyre_model_ekf"]["pass_0"]
c_alpha_f, c_alpha_r = cfg3["c_alpha_front_n_per_rad"], cfg3["c_alpha_rear_n_per_rad"]
mu_fz_f, mu_fz_r = cfg3["mu_fz_front_N"], cfg3["mu_fz_rear_N"]
...
# pass_2's own step vs pass_0 (recomputed here, not hand-copied, for the
# shrinking-step falsifiability check in predictions 1/2).
step_0_to_2 = {k: (prior2[k] - prior0[k]) / prior0[k] for k in current}
step_2_to_3 = {k: (current[k] - prior2[k]) / prior2[k] for k in current}

print("=" * 100)
print("SECTION 0 -- refitted parameters vs pass_2, relative change; step-size comparison "
      "vs the pass_0->pass_2 step (shrinking-step convergence check)")
print("=" * 100)
for key in current:
    same_sign = (step_0_to_2[key] > 0) == (step_2_to_3[key] > 0)
    shrinking = abs(step_2_to_3[key]) < abs(step_0_to_2[key])
    ...
    print(f"  {key}:")
    print(f"    pass_0={prior0[key]:12.2f}   pass_2={prior2[key]:12.2f}   pass_3={current[key]:12.2f}")
    print(f"    step pass_0->pass_2 = {step_0_to_2[key]*100:+7.2f}%   "
          f"step pass_2->pass_3 = {step_2_to_3[key]*100:+7.2f}%   [{mode}]")
print()

print("=" * 100)
print("SECTION 0b -- mu_fz/c_alpha ratio trend, pass_0 / pass_2 / pass_3 (ridge check)")
print("=" * 100)
for axle, c_key, m_key in (("front", "c_alpha_front_n_per_rad", "mu_fz_front_N"),
    ...
    r3 = current[m_key] / current[c_key]
    step_r_0_2 = (r2 - r0) / r0
    step_r_2_3 = (r3 - r2) / r2
    print(f"  {axle}: ratio pass_0={r0:.5f}  pass_2={r2:.5f}  pass_3={r3:.5f}   "
          f"step pass_0->pass_2={step_r_0_2*100:+.2f}%   step pass_2->pass_3={step_r_2_3*100:+.2f}%")
print()
...
innovation = result_3["innovation"][base_mask]
nis_combined = result_3["nis"][base_mask]
nis_yaw = innovation[:, 0] ** 2 / result_3["S_diag"][base_mask][:, 0]
nis_ay = innovation[:, 1] ** 2 / result_3["S_diag"][base_mask][:, 1]
...
s_m = state.get("s_m")
v_kmh = v * 3.6
beta_deg_full = np.degrees(beta_3)
...
# --- Section 3: h2-vs-ay, apex_3 population, pass_3's OWN alpha/curve ------

print("=" * 100)
print("SECTION 3 -- h2-vs-ay, apex_3 population (n~471), pass_3's OWN alpha and OWN curve")
print("=" * 100)
...
alpha_f_3 = slip_3["alpha_f_filt"]
alpha_r_3 = slip_3["alpha_r_filt"]
m_kg = params["vehicle"]["mass_kg"]

h2_pred_apex = np.full(len(idx), np.nan)
for k, i in enumerate(idx):
    Fy_f = dugoff_lateral_force(alpha_f_3[i], c_alpha_f, mu_fz_f)
    Fy_r = dugoff_lateral_force(alpha_r_3[i], c_alpha_r, mu_fz_r)
