# WP-N2 pass 3: Dugoff curve refit from the EKF's OWN slip angles
# (pass_2, the previous refit's own resulting configuration), continuing
# the kinematic-seeded-fit -> observer -> refit loop. Structurally
# identical to fit_dugoff_pass2_refit.py -- only EKF_SOURCE_PASS_ID and
# the manifest path differ -- kept as a separate file for the same
# reason pass_2 was: each numbered pass must stay reproducible from its
# own recorded parameters, and pass 2's manifest/config already point at
# fit_dugoff_pass2_refit.py specifically.
#
# Two-step fit per axle, same shape as WP-N1b/pass_2:
...
#      (pass_2's), over this script's base mask intersected with THAT
#      EKF-alpha CS_ratio's own ==1.0 flag.
#   2. mu_fz: 1-D nonlinear least squares on the full Dugoff form,
#      c_alpha fixed from step 1, using every masked sample. NOTE
#      (carried forward from pass 2's post-mortem, thesis_notes.md WP-N2
#      pass 3 entry): step 1 and step 2 are SEQUENCED, not independent --
#      mu_fz's optimum is sensitive to whatever c_alpha step 1 produced.
...
MANIFEST_PATH = "diagnostics/fit_dugoff_pass3_refit_manifest.json"
EKF_SOURCE_PASS_ID = "pass_2"
...
print("=" * 100)
print(f"WP-N2 pass 3 -- Dugoff refit, slip angles from EKF {EKF_SOURCE_PASS_ID}")
print("=" * 100)
...
manifest = {
    ...
    "c_alpha_source": "module_4b median over CS_ratio==1.0 linear-regime samples, "
                       ...
                       "(not the kinematic path's CS_ratio flag) -- WP-N2 pass 3",
